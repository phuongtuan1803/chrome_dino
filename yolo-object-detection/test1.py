...
while True:
	start = time.time()
	test_box = (100,100,1820,500)
	img = ImageGrab.grab(bbox=test_box) #x, y, w, h
	frame = np.array(img)
	...
	if type == -1:
		...
		type_str = LABELS[type]
	text = f'speed: {speed: 10.2f}.  type: {LABELS[type]}.  distance: {distance:10d}.  obj_height: {obj_height:10d}.  obj_width: {obj_width:10d}'
	cv2.putText(frame, text, (50, 50),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0) , 2)

	end = time.time()
	fps_value = 1 / (end -start)
	cv2.putText(frame, f'fps: {fps_value: 3.2f}', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
